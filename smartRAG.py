...
# Debug print to confirm (optional — remove later)
...
def initialize_session_state():
    ...
    if 'initialized' not in st.session_state:
        ...
        st.session_state.qdrant_url = "https://676bd76f-120d-4abe-a027-be6d298b8548.us-west-2-0.aws.cloud.qdrant.io"
        st.session_state.doc_url = "https://www.worldcancercongress.org/sites/default/files/2025-06/Factsheet%20General%20Cancer%20facts.pdf"  
...
def setup_sidebar():
    ...
    with st.sidebar:
        ...
        # 🔑 API key inputs with fallback to .env values
        st.session_state.openai_api_key = st.text_input(
            "OPENAI_API_KEY",
            value=st.session_state.openai_api_key or "",
            type="password",
            help="Required for gpt-4o-mini model"
        )

        st.session_state.tavily_api_key = st.text_input(
            "TAVILY_API_KEY",
            value=st.session_state.tavily_api_key or "",
            type="password"
        )
        ...
        st.session_state.doc_url = st.text_input(
            "DOCUMENT_URL",
            value=st.session_state.doc_url or ""
        )
        ...
        if not all([
            ...
            st.warning("⚠️ Please provide all required API keys and Qdrant URL")
            st.stop()
...
DOCUMENT_URL = st.session_state.doc_url
...
client = QdrantClient(
    url="https://676bd76f-120d-4abe-a027-be6d298b8548.us-west-2-0.aws.cloud.qdrant.io",
    api_key="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhY2Nlc3MiOiJtIn0.yZtBkhcGKdoM8VftYG5Mb8fss-4R3Uqiyja9uhp74hg"
)
...
if docs:
    ...
    try:
        ...
        print(f"Collection delete skipped or failed: {e}")
...
# Now try creating the collection
    ...
    BATCH_SIZE = 100  # You can adjust based on your data size and Qdrant performance
    ...
    for i in range(0, len(all_splits), BATCH_SIZE):
        ...
        try:
            ...
            time.sleep(2)  # Small delay before retrying or continuing
...
def generate(state):
    """Generate answer using gpt-4o-mini model"""
    print("~-generate-~")
    state_dict = state["keys"]
    ...
    try:
        ...
        llm = ChatOpenAI(model="gpt-4o-mini", api_key=st.session_state.openai_api_key,
...
if __name__ == "__setup_sidebar__":
    setup_sidebar()    
>>>>>>> 3762905cf6523949142dbe71823795b2b44019d6
